-- 069_grape_variety_repair_record_fix.sql
--
-- Patch migration that fixes a runtime crash in the grape variety repair
-- functions introduced in 067/068:
--
--   ERROR: 55000: record "v_match" is not assigned yet
--   DETAIL: The tuple structure of a not-yet-assigned record is indeterminate.
--   CONTEXT: SQL expression "v_match.key is not null"
--
-- Root cause:
--   Both `repair_grape_variety_allocations` and
--   `repair_grape_variety_allocations_admin` declared `v_match record`
--   and called:
--       select * into v_match from public._variety_catalog_match(v_name);
--   When `_variety_catalog_match(...)` returned zero rows, `v_match` was
--   left with no row-type assigned, so the subsequent
--   `if v_match.key is not null then` raised 55000 because PL/pgSQL cannot
--   inspect a column on a record whose tuple structure has never been
--   defined.
--
-- Fix:
--   Replace the unbound `record` variable with explicit scalar variables
--   (`v_match_key`, `v_match_display_name`) and use a typed
--   `select ... into ...` so the variables are simply NULL when no row
--   matches. The repair logic and idempotency are preserved.
--
-- This migration re-creates both functions with the corrected pattern.
-- Safe to run repeatedly; uses `create or replace function`.
    ...
    foreach v_vy_id in array v_scope_ids loop
        ...
        loop
            ...
            for v_idx in 0 .. jsonb_array_length(v_allocs) - 1 loop
                ...
                -- scalars so the variables are simply NULL when no row
                -- matches (instead of leaving an unbound record).
                v_match_key          := null;
